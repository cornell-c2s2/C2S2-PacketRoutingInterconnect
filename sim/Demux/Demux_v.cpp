...
#include "obj_dir_Demux/VDemux.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_Demux/VDemux__Syms.h"
#include "svdpi.h"
#endif
...
VDemux_t * create_model( const char *vcd_filename ) {
  ...
  m     = (VDemux_t *) malloc( sizeof(VDemux_t) );
  model = new VDemux();
  ...
  m->flattened_out_val = &model->flattened_out_val;
  m->in_val = &model->in_val;
  m->sel = &model->sel;
...
void destroy_model( VDemux_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VDemux * model = (VDemux *) m->model;
...
#if VLINETRACE
void trace( VDemux_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_Demux__v );
