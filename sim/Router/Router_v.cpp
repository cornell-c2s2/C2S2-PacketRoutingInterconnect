...
#include "obj_dir_Router/VRouter.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_Router/VRouter__Syms.h"
#include "svdpi.h"
#endif
...
VRouter_t * create_model( const char *vcd_filename ) {
  ...
  m     = (VRouter_t *) malloc( sizeof(VRouter_t) );
  model = new VRouter();
  ...
  m->message_in = &model->message_in;
  m->message_out = &model->message_out;
  m->ready = &model->ready;
  m->ready_out = &model->ready_out;
  m->valid = &model->valid;
  m->valid_out = &model->valid_out;
...
void destroy_model( VRouter_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VRouter * model = (VRouter *) m->model;
...
#if VLINETRACE
void trace( VRouter_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_Router__v );
